...
class WP_Deploy_Flow_Command extends WP_CLI_Command {
	 ...
	 * ## EXAMPLE
	 *
	 *    # Pushes the database and the uploads for to "staging" environment.
	...
	public function dump( $args, $assoc_args ) {
		...
		foreach ( $what as $item ) {
			if ( method_exists( __CLASS__, "_dump_{$item}" ) ) {
				call_user_func( "self::_dump_{$item}" );
			} else {
	...
	private function _push_uploads( $args = array() ) {
		...
		$local_path = trailingslashit( self::_launch( "cd {$uploads_dir['basedir']}; pwd -P;" ) );

		$settings = self::$_settings;
		$destination = empty( $args['safe'] ) ? "{$settings['ssh_user']}@{$settings['ssh_host']}:{$settings['uploads_path']}" : false;
		self::_rsync( $local_path, $destination );
	...
	private static function _dump_db( $dump_file = '' ) {
		...
		$env = self::$_env;
		$backup_file = self::_get_temp_bk( '.sql' );
		$abspath = untrailingslashit( ABSPATH );
		$siteurl = untrailingslashit( self::_trim_url( get_option( 'siteurl' ) ) );
		$path = untrailingslashit( self::$_settings['path'] );
		$url = untrailingslashit( self::$_settings['url'] );
		...
		$commands = array(
			...
			'export' => array( "wp db export $dump_file", true, 'Dumped the db which will be deployed.' ),
		...
		if ( ! isset( $commands[1] ) && ! isset( $commands[2] ) ) {
			$commands = array( $commands['export'] );
		}
	...
	private static function _pull_db( $cleanup = false, $backup = true ) {
		...
		$commands = array(
			array(
				"ssh $ssh_db_user@$ssh_db_host 'mkdir -p $ssh_db_path; cd $ssh_db_path; mysqldump --user=$db_user --password=$db_password --host=$db_host --add-drop-table $db_name > $server_file'",
		...
		$path = untrailingslashit( $_settings['path'] );
		$abspath = unstrailingslashit( $path );
		if ( $path != $abspath ) {
			self::_run_commands( array( "wp search-replace --network $path $abspath", true, "Replaced $path with with $abspath on local db." ) );
		}
	}

	private static function _pull_uploads( $cleanup = false, $backup = false ) {

		$backup_name = date( 'Y_m_d-H_i' ) . '_uploads_bk';

		WP_CLI::line( "\n=Deploying the uploads to server." );

		/**
		 * The source trailing slash ensures the content will be copied and
		 * not the dir itself.
		 */
		$source = trailingslashit( self::$_settings['uploads_path'] );

		$uploads_dir = wp_upload_dir();
		$local_path = self::_launch( "cd {$uploads_dir['basedir']}; pwd -P;" );
		/* $local_path = ABSPATH . "{$env}_pull_" . self::_get_unique_env_id(); */

		//backup
		if ( $backup ) {
			$pull_dir = "{$env}_pull_" . self::_get_unique_env_id();
			$pull_path = ABSPATH . $pull_dir;
			self::_run_commands( array(
				array( "mkdir -p $pull_path" ),
				array(
					"cp $local_path $pull_path/$backup_name",
					true, "Backed up the uploads db to $pull_dir/$backup_name"
				)
			) );
		}
	...
	private static function _get_sanitized_args( $args, $assoc_args = null ) {
		...
		$out['backup'] = isset( $assoc_args['backup'] ) ? true : false;
	...
	private static function _get_wd( $path = true ) {
		$env = self::$_env;
		$dir_name = "{$env}_" . self::_get_unique_env_id();
		$wd_path = trailingslashit( ABSPATH ) . $dir_name;
		if ( $path === true ) {
			return $wd_path;
		} elseif ( $path ) {
			return trailingslashit( $wd_path ) . $path;
		}
